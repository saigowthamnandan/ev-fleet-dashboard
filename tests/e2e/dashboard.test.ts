...
test.describe('Vehicle Dashboard - Essential Tests', () => {
  ...
  test('should load dashboard with 12 vehicles and all main components', async ({page}) => {
  ...
  test('should filter vehicles by status', async ({page}) => {
    ...
    const filteredCount = await page.locator('[data-testid="vehicle-card"]').count();
  ...
  test('should toggle online/offline status and show alerts', async ({page}) => {
    ...
    await connectionToggle.click();
    await expect(connectionToggle).toContainText('Online');
    await expect(page.locator('[data-testid="offline-alert"]')).not.toBeVisible();
  ...
  test('should handle real-time updates and maintain responsiveness', async ({page}) => {
    ...
    // Get initial state
    const initialBattery = await page.locator('[data-testid="vehicle-battery"]').first().textContent();
    ...
    const currentBattery = await page.locator('[data-testid="vehicle-battery"]').first().textContent();
